...
using 30s FEMALE mel-spectrogram segments (full dataset).

Data:
  - MEL_DIR (female 30s mel_spectrum)
  - LABEL_DIR (segment labels)

Pipeline:
  - Pair mel npy files with *_segments_labels.npy
  - Patient-level (file-level) split: 70% train, 20% val, 10% test
  - Each segment: (C, n_mels, frames)
      -> average-pool time by TIME_POOL
      -> sequence of length T' with feature dim = C * n_mels
  - Tiny Transformer encoder over time
  - Binary classification: Normal (0) vs Event (1)
"""
...
OUT_DIR = "/scratch/jbfrantz/jess_explainability_model/jess_outputs"
os.makedirs(OUT_DIR, exist_ok=True)
MODEL_PATH = os.path.join(OUT_DIR, "best_female_transformer.pt")
METADATA_PATH = os.path.join(OUT_DIR, "dataset_metadata.npz")
...
def fast_compute_pos_weight(dataset):
    # NEW: Use the flat labels list instead of label_arrays
    all_labels = []
    
    if dataset.label_to_int is not None:
        # Convert string labels to binary
        for lab in dataset.labels:
            all_labels.append(dataset.label_to_int[str(lab)])
    else:
        # Already numeric
        all_labels = [float(lab) for lab in dataset.labels]
...
def main():
    ...
    # 1) Pair female mel with labels
    pairs_all = find_file_pairs(MEL_DIR, LABEL_DIR)

    # 2) Patient-level split over ALL female patients
    ...
    # 4) pos_weight from full train dataset
    pos_weight = fast_compute_pos_weight(train_dataset).to(DEVICE)

    # 5) infer feature dimension
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if val_auc is not None and val_auc > best_val_auc:
            ...
            print(f"[INFO] New best val AUC (female): {best_val_auc:.4f} at epoch {epoch}")

        plot_confusion_matrix(
            val_cm,
            class_names=["Normal (0)", "Event (1)"],
            filename=f"confusion_matrix_val_female_transformer_epoch{epoch}.png",
            title=f"Female Val Confusion Matrix (Epoch {epoch})",
        )
    ...
    if best_state is not None:
        ...
        plot_confusion_matrix(
            test_cm,
            class_names=["Normal (0)", "Event (1)"],
            filename="confusion_matrix_test_female_transformer.png",
            title="Female Test Confusion Matrix (Tiny Transformer, 30s mel)",
        )
